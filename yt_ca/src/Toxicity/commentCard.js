...
const CommentCard = ({user, comment, toxicity}) => {
  return (
    <table className="card-header py-3 d-flex flex-row align-items-center justify-content-between">
        <tc className="m-0 font-weight-bold" style={{width:"20%",'color':'red'}}>{user}</tc>
        <tc className="m-0 font-weight-bold" style={{width:"60%",'color':'black','text-align':'left'}}>{comment}</tc>
        {toxicity.map((c, idx)=>
            (
